...
def store_instacart(log_train):
    try:
        connection = psycopg2.connect(database="auto_fg",
                                      user="lydiazheng",
                                      password="000001",
                                      host="127.0.0.1",
                                      port="5432")
        ...
        # Create user_info
        cursor.execute("CREATE TABLE log_train (user_id integer NOT NULL, \
        ...
        engine = create_engine(
            'postgresql://lydiazheng:@127.0.0.1:5432/auto_fg', echo=False)
        log_train.to_sql('log_train', con=engine,
...
def generate_feature_in_small_space(engine, agg, m, lb_day, ub_day, w_cat, w_cat_value):
    ...
    if len(w_cat) != 0:
        for i in range(len(w_cat)):
            w_cat_str = w_cat_str + \
                str(w_cat[i]) + '=' + str(w_cat_value[i]) + ' AND '

    return engine.execute("SELECT user_id, "+str(agg)+"("+str(m)+") \
                   FROM log_train \
                   WHERE " + w_cat_str + "\
                       days_since_prior_order BETWEEN "+str(lb_day)+" AND "+str(ub_day)+"\
...
def model(user_train):
    ...
    accuracy = cross_validate(clf, X, y, cv=5,scoring=('accuracy'), return_train_score=True, n_jobs=-1, return_estimator=True)
...
########## Tmall exp #########
# generate features for pre-defined attributes
def generate_feature_in_small_space_tmall(engine, agg, m, lb_day, ub_day, w_cat, w_cat_value):
    w_cat_str = ''
    if len(w_cat) != 0:
        for i in range(len(w_cat)):
            w_cat_str = w_cat_str + \
    ...
    return engine.execute("SELECT user_id, "+str(agg)+"("+str(m)+") \
                   FROM tmall_log \
                   WHERE " + w_cat_str + "\
                       time_stamp BETWEEN "+str(lb_day)+" AND "+str(ub_day)+"\
                   GROUP BY user_id").fetchall()
...
def model_tmall(user_train):
    ...
    return valid_auc, auc['estimator'][0]
...
def store_tmall(tmall_log):
    try:
        ...
        # Create user_info
        cursor.execute("CREATE TABLE tmall_log (user_id integer NOT NULL, \
        ...
        engine = create_engine(
            'postgresql://lydiazheng:@127.0.0.1:5432/auto_fg', echo=False)
        tmall_log.to_sql('tmall_log', con=engine,
                         if_exists='replace', index=False)
    except (Exception, psycopg2.Error) as error:
        print("Error while connecting to PostgreSQL", error)
    finally:
