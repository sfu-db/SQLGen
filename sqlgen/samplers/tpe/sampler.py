...
from sqlgen import distributions
from sqlgen.samplers import base
from sqlgen.samplers import random
from sqlgen.samplers.tpe.parzen_estimator import _ParzenEstimator
from sqlgen.samplers.tpe.parzen_estimator import _ParzenEstimatorParameters
from sqlgen.study import StudyDirection
...
if type_checking.TYPE_CHECKING:
    ...
    from sqlgen.distributions import BaseDistribution  # NOQA
    from sqlgen.study import Study  # NOQA
    from sqlgen.trial import FrozenTrial  # NOQA
...
class TPESampler(base.BaseSampler):
    ...
    def _split_observation_pairs(
        ...
        loss_vals = np.asarray(
            loss_vals, dtype=[("step", float), ("score", float)])
    ...
    def _sample_discrete_uniform(self, distribution, below, above):
        ...
        best_sample = self._sample_numerical(
            low, high, below, above, q=q) + distribution.low
    ...
    def _sample_int_loguniform(self, distribution, below, above):
        ...
        best_sample = (
            np.round((sample - distribution.low) /
                     distribution.step) * distribution.step
            + distribution.low
        )
    ...
    def _sample_categorical_index(self, distribution, below, above):
        ...
        counts_below = np.bincount(
            below, minlength=upper, weights=weights_below)
        ...
        counts_above = np.bincount(
            above, minlength=upper, weights=weights_above)
    ...
    def _sample_from_gmm(
        ...
        active = np.argmax(self._rng.multinomial(
            1, weights, size=size), axis=-1)
    ...
    def _gmm_log_pdf(
        ...
        if mus.ndim != 1:
            raise ValueError(
                "The 'mus' should be 2-dimension. " "But mus.shape = {}".format(
                    mus.shape)
            )
        if sigmas.ndim != 1:
            raise ValueError(
                "The 'sigmas' should be 2-dimension. " "But sigmas.shape = {}".format(
                    sigmas.shape)
            )
        ...
        if q is None:
            ...
            probabilities = np.sum(
                weights[..., None]
                * (
                    cdf_func(upper_bound[None],
                             mus[..., None], sigmas[..., None])
                    - cdf_func(lower_bound[None],
                               mus[..., None], sigmas[..., None])
                ),
                axis=0,
            )
    ...
    def _sample_from_categorical_dist(self, probabilities, size):
        ...
        sample = self._rng.multinomial(
            n=1, pvals=probabilities, size=int(n_draws))
