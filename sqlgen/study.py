...
from sqlgen._experimental import experimental
from sqlgen._study_direction import StudyDirection
from sqlgen._study_summary import StudySummary  # NOQA
...
from sqlgen import exceptions
from sqlgen import logging
from sqlgen import progress_bar as pbar_module
from sqlgen import pruners
from sqlgen import samplers
from sqlgen import storages
from sqlgen import trial as trial_module
from sqlgen.trial import FrozenTrial
from sqlgen.trial import TrialState
from sqlgen.storages.in_memory import InMemoryStorage
from sqlgen import type_checking
...
class Study(BaseStudy):
    ...
    def optimize(
        ...
        # type: Optional[List[Callable[[Study, FrozenTrial], None]]]
        callbacks=None,
            ...
            if n_jobs == 1:
                if (mi_initializer):
                    self._optimize_sequential(
                        ...
                        mi_initializer, evaluate, train_size, mi_init_trails, tpe_startup_trails)
                else:
                    self._optimize_sequential(
                        func, n_trials, timeout, catch, callbacks, gc_after_trial, None)
            else:
    ...
    def trials_dataframe(
        ...
        records = list([_create_record_and_aggregate_column(trial)
                        for trial in trials])
    ...
    def _append_trial(
        ...
        trial_id = self._storage.create_new_trial(
            self._study_id, template_trial=trial)
        return trial_id
    ...
    def _optimize_sequential(
        ...
        '''
        if i_trial < tpe_startup_trails:
            self._initialize_with_mi(
                mi_initializer, evaluate, catch, gc_after_trial, mi_init_trails, tpe_startup_trails)
            i_trial += tpe_startup_trails
        '''

        mi_log, accuracy_log, args = [], [], []
        ...
        while True:
            ...
            if(mi_initializer):
                ...
                mi_initializer, catch, callbacks, gc_after_trial, mi_log=mi_log, args=args)
            else:
        ...
        if (evaluate):
            accuracy_log, best_trial_id = evaluate(args, mi_log, train_size)

            for trial_id in range(mi_init_trails):
                self._storage.set_trial_value(trial_id, accuracy_log[trial_id])

            # update cache with highest accuracy value of labeled training data
            study_id = self._storage.get_study_id_from_trial_id(best_trial_id)
            self._storage._update_cache(best_trial_id, study_id)
    ...
    def _initialize_with_mi(
        ...
        print(mi_startup_trails, tpe_startup_trails)
        trials = []
        mi_scores = {}
        for i in range(0, mi_startup_trails):
            trial_id = self._pop_waiting_trial_id()
            if trial_id is None:
                trial_id = self._storage.create_new_trial(self._study_id)
            trial = trial_module.Trial(self, trial_id)
            mi_score = mi_initializer(trial)
            trials.append(trial)
            mi_scores[i] = mi_score

        ordered_mi_scores = sorted(
            mi_scores.items(),  key=lambda item: item[1], reverse=True)

        for i in range(tpe_startup_trails):
            # self._storage.change_trial_id(ordered_mi_scores[i][0], i)
            trial = trials[ordered_mi_scores[i][0]]
            trial_id = ordered_mi_scores[i][0]
            trial_number = trial.number
            print(trial.params, "mi score: ", str(ordered_mi_scores[i][1]))
            try:
                # score = evaluation(trial)
            ...
            if math.isnan(result):
                message = (
                    ...
                    "returned {}.".format(
                        trial_number, TrialState.FAIL, result)
                )
                _logger.warning(message)
                self._storage.set_trial_system_attr(
                    trial_id, "fail_reason", message)
        ...
        for i in range(tpe_startup_trails, mi_startup_trails):
            self._storage.set_trial_state(
                ordered_mi_scores[i][0], TrialState.FAIL)
    ...
    def _pop_waiting_trial_id(self):
        ...
        for trial in self.get_trials(deepcopy=False):
            ...
            _logger.debug(
                "Trial#{} is popped from the trial queue.".format(trial.number))
    ...
    def _run_trial_and_callbacks(
        ...
        trial = self._run_trial(func, catch, gc_after_trial, mi_log, args)
        if callbacks is not None:
            frozen_trial = copy.deepcopy(
                self._storage.get_trial(trial._trial_id))
    ...
    def _run_trial(
        ...
        try:
            result = func(trial)
            if args is not None and mi_log is not None:
                args.append(trial.params)
                mi_log.append(result)
        except exceptions.TrialPruned as e:
            message = "Setting status of trial#{} as {}. {}".format(
                trial_number, TrialState.PRUNED, str(e)
            )
...
def load_study(
    study_name,  # type: str
    storage=InMemoryStorage(),  # type: Union[str, storages.BaseStorage]
    sampler=None,  # type: samplers.BaseSampler
    pruner=None,  # type: pruners.BasePruner
):
    # type: (...) -> Study
...
def get_all_study_summaries(storage=InMemoryStorage()):
